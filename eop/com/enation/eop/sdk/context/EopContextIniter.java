...
public class EopContextIniter {
	...
	public static void init(HttpServletRequest httpRequest, HttpServletResponse httpResponse) {
		...
		String session_id = httpRequest.getSession().getId();
		//if(! httpRequest.getRemoteAddr().contains("0:0") && ! httpRequest.getRemoteAddr().contains("127.0.0.1")){
			if(servletPath.endsWith(".html") | servletPath.endsWith("/") && !servletPath.endsWith("/common/my_store.html")){	
				if(allianceCountManager.haveSession(session_id).size() > 0){
					allianceCountManager.editAllianceAction(servletPath, session_id);
				}else{
					allianceCountManager.addSession_id(session_id);
				}
			}*/
		//}
