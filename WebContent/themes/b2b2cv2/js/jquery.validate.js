...
(function($){
	...
	var DefLang ={
		...
		$.isEmail=function(val){var reg = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/; return reg.test(val);};	
		...
		$.isIdNumber=function(val){var reg = /(^\d{18}$)|(^\d{17}(\d|X|x)$)/; return reg.test(val);};
		...
		dt_id_number = function()
		{
		  this.check = function (val) {if(val=='') return true; return /(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(val);};
		  this.errorMsg = function () {if (validator_lang_exist('is_not_id_number')) {return DefLang.is_not_id_number} else {return "this value is not id Number!"}};
		};	
		var opts;
	var Validator={
			types :{"chinese":new dt_chinese(),"string":new dt_string(),"int":new dt_int(),"date":new dt_date(), "email":new dt_email,"tel_num":new dt_tel_num(),"id_num":new dt_id_number(),
