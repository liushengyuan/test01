...
var CheckOut={
	...
	init:function(){
		...
		$("#createBtn").click(function(){
			...
			if(modifyWrapper.html()==null){
				alert(" Пожалуйста, выберите  получение  информации ");
				return false;
			}
	...
	changeBonus:function(bonusid){
		...
		$.ajax({
			...
			error:function(){
				alert(" плохо,  использовать  купон  неожиданная ошибка ");
			}
		});
	},
	 ...
	 whenOpen:function(){
		 ...
		 $(".bonusADD").click(function(){
			 ...
			 if($(".bonusSPAN>div").size()>=5){
				 alert(" использовать максимум  5  конверт ");
				 return false;
			 }
	...
	bindUseEvent:function(){
		...
		$(".bonusSPAN .userY").unbind("click").bind("click",function(){
			...
			var sn  = ipt.val();
			if(sn==""){
				alert( " Пожалуйста, введите  купон  номер " );
				return ;
			}
			var count =0; 
	...
	bindDlytypeEvent:function(){
		...
		$("#saveDlytypeBtn").click(function(){
			...
			if(dlytype.size()==0){
				$.alert("Пожалуйста, выберите  способ распределения ");
				return false;
			}
	...
	createOrder:function(btn){
		...
		btn.addClass("disabled");
 		$.Loading.show("В настоящее время  представить  ваш заказ  ,  пожалуйста, подождите."); 
		var options = {
			...
			success : function(result) {
 				if(result.result==1){
	 				...
	 				$.Loading.hide();
	 				$.alert("Заказать не успешно, закажите ещё раз, пожалуйста!");//result.message
