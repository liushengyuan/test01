...
public class StoreGoodsTagManager implements IStoreGoodsTagManager {
	...
	public List list(Integer store_id) {
		...
		List list1 = daoSupport.queryForList(sql, store_id);
		if(list1.size()>0){
			return list1;
		}else{
			return null;
	...
	public Page getGoodsList(int tagid, int page, int pageSize) {
		...
		sql.append("select g.goods_id,g.name,g.name_ru,r.tag_id,r.ordernum,g.price,g.price_ru,g.buy_num from es_tag_rel r LEFT JOIN es_goods g ON g.goods_id=r.rel_id where g.disabled=0 and g.market_enable=1");
	...
	public Page getGoodsList(Map map, int page, int pageSize) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select g.goods_id,g.name,g.name_ru,r.tag_id,r.ordernum,g.price,g.price_ru,g.buy_num,g.thumbnail,g.small from es_tag_rel r LEFT JOIN es_goods g ON g.goods_id=r.rel_id where g.disabled=0 and g.market_enable=1");
		sql.append(" and r.tag_id=? ");
