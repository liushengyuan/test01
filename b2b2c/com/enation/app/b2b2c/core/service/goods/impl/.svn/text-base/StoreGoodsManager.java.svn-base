...
public class StoreGoodsManager extends BaseSupport implements IStoreGoodsManager{
	...
	public List<Map> storeGoodsList(int storeid, Map map) {
		...
		if(!StringUtil.isEmpty(keyword) ){
			if (language=="zh") {
				...
				sql.append(" and ((g.name_ru like '%"+keyword+"%') or (g.sn like '%"+keyword+"%') )");
			}
		}
		if(map.get("is_groupbuy")!=null){
			sql.append(" and g.is_groupbuy!=1");
			sql.append(" and g.market_enable=1");//搜索已上架的产品
		}
	...
	public Page store_searchGoodsList(Integer page, Integer pageSize, Map params) {
		...
		StringBuffer sql=new StringBuffer("select g.name,g.name_ru,g.goods_id,g.price,g.price_ru,g.thumbnail,g.big,g.disabled,g.market_enable,g.store_cat_id,g.small,g.original from es_goods g where disabled=0 and market_enable=1 and store_id="+storeid);
		if(language.equals("zh")){
			...
			if(!StringUtil.isEmpty(keyword)){
				sql.append("  and g.name_ru like '%"+keyword+"%' ");
			}
			
			if(!StringUtil.isEmpty(start_price)){
				sql.append(" and price_ru>="+ Double.valueOf(start_price));
			}
			if(!StringUtil.isEmpty(end_price)){
	 			sql.append(" and price_ru<="+ Double.valueOf(end_price));
			}
		}
	...
	@Override
	public void setGoodsNum(int goodsId) {
		// TODO Auto-generated method stub
		StoreGoods goods=this.getGoods(goodsId);
		String sql = "select count(0) from goods where market_enable=1 and  disabled = 0 and store_id=?";
		int sellcount = this.baseDaoSupport.queryForInt(sql,goods.getStore_id());
		this.daoSupport.execute("update es_store set goods_num=? where store_id=?",sellcount,goods.getStore_id());
	}

	@Override
	public List getstoregoods(Integer goodsId, Integer productId) {
		StringBuffer sql = new StringBuffer();
		sql.append(" select * from es_product_store s where s.goodsid=? and s.productid=? ");
		List list = this.baseDaoSupport.queryForList(sql.toString(), goodsId,productId);
		return list;
	}
