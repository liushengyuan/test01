 ...
 */
@Component
public class StoreCheckCartGoodsTag extends BaseFreeMarkerTag{
	...
	protected Object exec(Map params) throws TemplateModelException {
		...
		ArrayList<Integer> storecount_id=new ArrayList<Integer>();
		...
		for(Map map : storeGoodsList){
			...
			OrderPrice orderPrice =  this.storeCartManager.countPriceByBonus(store_id,list, regionsid+"", type_id, false ,null);
			//获取购物车列表数量
			Integer count =storeGoodsList.size();
			double everyDiscountPrice=0.0;
			if(memberbonus.getSendplat()!=null){
				if(list.size()==datacount){
					if(bonusnumber!=0){
						 everyDiscountPrice = CurrencyUtil.div(orderPrice.getDiscountPrice(), count-bonusnumber);
						 map.put("discountprice", everyDiscountPrice);
					}else{
						everyDiscountPrice = CurrencyUtil.div(orderPrice.getDiscountPrice(), count);
						 map.put("discountprice", everyDiscountPrice);
					}
				}else{
				   storecount_id.add(store_id);
