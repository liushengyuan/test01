	...
	public void add(List<Product> productList) {
		...
		for (Product product : productList) {
			...
			for (SpecValue specvalue : specList) {
				if(specvalue.getSpec_id()==1000){
					this.daoSupport
					.execute(
							"insert into "
									+ this.getTableName("goods_spec")
									+ "(spec_id,spec_value_id,goods_id,product_id,spec_img)values(?,?,?,?,?)",
							specvalue.getSpec_id(),
							specvalue.getSpec_value_id(),
							product.getGoods_id(), product_id,specvalue.getSpec_image());
				}else{
					this.daoSupport
					.execute(
							"insert into "
...
/*				this.daoSupport
	...
	public List<Specification> listSpecs(Integer goodsId) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select distinct s.spec_id,s.spec_type,s.spec_name,s.spec_name_ru,sv.spec_value_id,sv.spec_value,sv.spec_value_ru,gs.spec_img as spec_img ,gs.goods_id as goods_id ");
		sql.append(" from ");
		...
		sql.append("where s.spec_id = sv.spec_id  and gs.spec_value_id = sv.spec_value_id and gs.goods_id=?  ORDER BY s.spec_id,sv.spec_order");
		...
		for (Map map : list) {
			...
			if (spec_id.intValue() != dbspecid.intValue()) {
				...
				spec.setSpec_name_ru(map.get("spec_name_ru").toString());
				spec.setSpec_type(Integer.parseInt(map.get("spec_type")
						.toString()));
			...
			value.setSpec_value_ru(map.get("spec_value_ru").toString());
			value.setSpec_value_id(Integer.valueOf(map.get("spec_value_id")
					.toString()));
			String spec_img = (String) map.get("spec_img");
	...
	public List<Product> list(Integer goodsId) {
		...
		sql = "select sv.*,gs.product_id product_id from  "
				...
				+ "  sv on gs.spec_value_id = sv.spec_value_id where  gs.goods_id=? order by gs.id desc";
		...
		for (Product pro : prolist) {
			...
			for (int i = size; i >= 0; i--) {
				...
				if (pro.getProduct_id().intValue() == productid) {
					...
					spec.setSpec_value_ru((String)gs.get("spec_value_ru"));
					spec.setSpec_type(Integer.parseInt(gs.get("spec_type")
							.toString()));
					pro.addSpec(spec);
				}
