...
//		pointHistoryManager.addPointHistory(pointHistory);
	...
	public Double getItemPoint2(String itemname) {
		String value = settingService.getSetting("point", itemname);
		if(StringUtil.isEmpty(value)){
			return (double) 0;
		}
	...
	@Override
	public List<RegisterSendPoint> getAllRegisterSendPoint() {
		long time = new Date().getTime()/1000;
		String sql = "SELECT * FROM es_register_point WHERE status = 1 and (start_time <=? and ?< end_time)";
		List<RegisterSendPoint> list = this.baseDaoSupport.queryForList(sql, RegisterSendPoint.class, time,time);
		return list;
	}
	@Override
	 public  List<Order>   getItemorder(int   orderid){
		 String sql ="select   *  from    es_order  where  parent_id=?  ";
		 List<Order> list=   this.baseDaoSupport.queryForList(sql,Order.class ,orderid);
		 return    list;
	 }
