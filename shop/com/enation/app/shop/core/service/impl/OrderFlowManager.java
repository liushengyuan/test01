	...
	public void shipping(Delivery delivery, List<DeliveryItem> itemList) {
		...
		if(delivery.getLogi_id()!=null && delivery.getLogi_id()!=0){
			...
			if(logi!=null){
				...
				delivery.setLogi_name("系统顺丰");
				delivery.setLogi_code("-2");
			}
...
//		}
	...
	public void rogConfirm(int orderId,Integer op_id,String op_name,String sign_name,Long sign_time){
		...
		if(order.getMember_id()!=null){
			...
			if(order.getPay_status() == OrderStatus.PAY_CONFIRM){
				//2014年10月17日，改为确认收货立即解冻积分//2016年2月17日，解冻积分由定时任务完成
	 ...
	 */
	@Override
	public void restitutionPoints(String sn) {
		//查找主订单
		Order order = this.getParentOrder(sn);
		//查找对应主订单的积分历史记录
		PointHistory pointHistory = new PointHistory();
		List<Order> orderList = null;
		if(order!=null){
			pointHistory = this.getPointHistory(order);
			orderList = this.orderList(order);
		}
		//此订单的所有的消费积分
		Integer mp = null;
		//总共所拥有的订单数
		Double countOrder = (double) 0;
		//每个订单对应的积分数
		Double backmp=(double) 0;
		if(pointHistory!=null){
			mp = pointHistory.getMp();
		}
		if(orderList!=null){
			countOrder = (double)orderList.size();
		}
		if(countOrder!=null && mp!=null){
			backmp = CurrencyUtil.div(mp,countOrder);
		}
		//如果主订单存在，对积分历史记录进行插入新的返回积分值，并更新相对应的会员积分
		if(order!=null && backmp>0){
			this.creatpointHistory(backmp, order);
		}
		
	}
	/**
	 * 对返回积分进行插入积分历史记录，并更新会员的积分
	 * @param backmp
	 * @param order
	 */
	private void creatpointHistory(Double backmp, Order order) {
		PointHistory pointHistory = new PointHistory();
		pointHistory.setMember_id(order.getMember_id());
		pointHistory.setOperator("member");
		Member member = this.getMember(order);
		if(member!=null){
		...
		int index = backmp.toString().indexOf(".");
		pointHistory.setMp(Integer.parseInt(backmp.toString().substring(0,index)));
		String sql = " update es_member set mp=mp+? where member_id=? ";
		this.baseDaoSupport.execute(sql, Integer.parseInt(backmp.toString().substring(0,index)),member.getMember_id());
	...
	private Member getMember(Order order) {
		...
		sql.append(" select * from es_member m where m.member_id=? ");
		Member member = (Member) this.daoSupport.queryForObject(sql.toString(), Member.class,order.getMember_id());
		if(member!=null){
			return member;
		}else{
			return null;
	...
	private List<Order> orderList(Order order) {
		StringBuffer sql = new StringBuffer();
		sql.append(" select * from es_order o where o.parent_id=? ");
		List<Order> orderlist = this.daoSupport.queryForList(sql.toString(), order.getOrder_id());
		if(orderlist.size()>0){
			return orderlist;
		}else{
			return null;
	...
	/**
	 * 根据主订单order,查找积分历史记录所对应的积分历史
	 * @param order
	 * @return
	 */
	private PointHistory getPointHistory(Order order) {
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT * from es_point_history p where p.order_id=? ");
		PointHistory pointHistory = (PointHistory) this.daoSupport.queryForObject(sql.toString(), PointHistory.class, order.getOrder_id());
		if(pointHistory!=null){
			return pointHistory;
		}else{
			return null;
	 ...
	 * @return
	 */
	private Order getParentOrder(String sn) {
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT * from es_order o where o.order_id in ( ");
		sql.append(" SELECT r.parent_id from es_order r where r.sn=?) ");
		Order order = (Order) this.daoSupport.queryForObject(sql.toString(), Order.class, sn);
		if(order!=null){
			return order;
		}else{
			return null;
