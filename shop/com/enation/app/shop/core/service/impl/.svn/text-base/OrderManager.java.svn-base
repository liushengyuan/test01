...
public class OrderManager extends BaseSupport implements IOrderManager {
	...
	public Order add(Order order, String sessionid) {
		...
		//获取使用的平台优惠券金额
		double bonusMoney = BonusSession.getUseMoney(order.getCurrency());
		order.setBonusMoney(bonusMoney);
		...
		/************ 写入订单货物列表 ************************/
		List<CartItem> itemList = this.cartManager.listGoodsOrder(sessionid);
		...
		if(pointprice!=null && !StringUtil.isEmpty(pointprice) && integralprice>0){
			this.creatpointHistory(integralprice,order,member,first);
			first=1;
		}
		/**
	...
	private void creatpointHistory(Double integralprice, Order order,
			Member member, int first) {
...
//		String language = locale.getLanguage();
		...
		String language = cartItem.getCurrency();
		if(first==0){
			PointHistory pointHistory = new PointHistory();
			pointHistory.setMember_id(member.getMember_id());
			pointHistory.setOperator("member");
			pointHistory.setPoint(member.getPoint());
			Date today = new Date();
			pointHistory.setReason(DateUtil.toString(today, "yyyy年MM月dd日")+"消费");
			pointHistory.setTime(DateUtil.getDateline());
			pointHistory.setRelated_id(null);
			pointHistory.setType(0);
			pointHistory.setOrder_id(order.getOrder_id());
			Double mp=(double) 0;
			if(language.equals("CNY")){
				PointConsume consume1 = this.pointConsumeManager.getCNYconsume();
				if(consume1!=null){
					//mp=integralprice/consume1.getConsume_amount();
					mp=CurrencyUtil.div(integralprice, consume1.getConsume_amount());
				}
			}
			if(language.equals("RUB")){
				PointConsume consume2 = this.pointConsumeManager.getRUBconsume();
				if(consume2!=null){
					//mp=integralprice/consume2.getConsume_amount();
					mp=CurrencyUtil.div(integralprice, consume2.getConsume_amount());
				}
			}
			int index = mp.toString().indexOf(".");
			pointHistory.setMp(Integer.parseInt(mp.toString().substring(0,index)));
			String sql = " update es_member set mp=mp-? where member_id=? ";
			this.baseDaoSupport.execute(sql, Integer.parseInt(mp.toString().substring(0,index)),member.getMember_id());
			
			this.baseDaoSupport.insert("es_point_history", pointHistory);
		}
	...
	private void saveGoodsItem(List<CartItem> itemList, Order order) {
		...
		for (int i = 0; i < itemList.size(); i++) {
			...
			//System.out.println(cartItem.getFreight_id()+"***"+cartItem.getSendprice());
			// 3.0新增的三个字段
	 ...
	 * @return
	 */
	public List<OrderItem> listGoodsItems1(Integer orderId) {
	...
	public Page listOrderByMemberId(int pageNo,int pageSize,int member_id) {
		String sql = "select o.order_id,o.ship_name,o.sn,o.status,o.pay_status,o.create_time from es_order o where member_id = ? and parent_id is not null order by create_time desc";
		Page list = this.baseDaoSupport.queryForPage(sql, pageNo, pageSize, Order.class, member_id);
		return list;
	}
	 ...
	 * 
	 * author zhanglibo
	 * @see com.enation.app.shop.core.service.IOrderManager#mapOrderByMemberId(int)
	 */
	public List<Member> listMemberByStoreId(int store_id) {
		String sql = "select * from es_member where member_id in (select DISTINCT member_id from es_order where store_id = ?)";
		List list = this.baseDaoSupport
				.queryForList(sql, Member.class, store_id);
		return list;
	}
	...
	@Override
	public void editNeedPayMoney(int order_id, Double money) {
		String sql = "UPDATE es_order SET need_pay_money = ? WHERE order_id = ?";
		this.baseDaoSupport.execute(sql, money,order_id);
		
	}

	@Override
	public void subNeedPayMoney(int order_id, Double money) {
		String sql = "UPDATE es_order SET need_pay_money = need_pay_money - ? ,discount = ? WHERE order_id = ?";
		this.baseDaoSupport.execute(sql, money,money,order_id);
	}


	@Override
	public void modifyOrderPrice(Order order) {
		if(order.getSn() == null) {
			throw new IllegalStateException("缺少订单号");
		}
		Order oldOrder = (Order) this.baseDaoSupport.queryForObject("SELECT * FROM order WHERE sn = ? ", Order.class, order.getSn());
		Double gooodsSub = 0D,shipSub = 0D;
		Map<String, Object> fields = new HashMap<String, Object>();
		
		if(order.getGoods_amount() != null) {
			 if(order.getShipping_amount()!=null){
			//商品价格
			fields.put("goods_amount", order.getGoods_amount());
			fields.put("shipping_amount", order.getShipping_amount());
			fields.put("order_amount", order.getGoods_amount() );
			gooodsSub=CurrencyUtil.sub(CurrencyUtil.add(order.getGoods_amount(), order.getShipping_amount()),oldOrder.getDiscount());
			fields.put("need_pay_money", gooodsSub);
			String sqlPayMent = "UPDATE  es_payment_logs  SET money = "+gooodsSub+" WHERE order_sn = ?";
			this.baseDaoSupport.execute(sqlPayMent, order.getSn());
			this.baseDaoSupport.update("order", fields, " sn = " + order.getSn());
			 }
		}	
		/*if(order.getShipping_amount() != null) {
			//配送价格
			fields.put("shipping_amount", order.getShipping_amount());
			shipSub = CurrencyUtil.sub(order.getShipping_amount(), oldOrder.getShipping_amount());
		}
		if(order.getGoods_amount() != null && order.getShipping_amount() != null) {
			//应付和已付
			Double result = CurrencyUtil.add(CurrencyUtil.add(gooodsSub, shipSub), oldOrder.getOrder_amount());
			fields.put("order_amount", result);
			result = CurrencyUtil.add(CurrencyUtil.add(gooodsSub, shipSub), oldOrder.getNeed_pay_money());
			fields.put("need_pay_money", result);
			//修改支付记录表里面的金额
			String sqlPayMent = "UPDATE  es_payment_logs  SET money = "+result+" WHERE order_sn = ?";
			this.baseDaoSupport.execute(sqlPayMent, order.getSn());
		}
		this.baseDaoSupport.update("order", fields, " sn = " + order.getSn());*/
	}
	...
	@Override
	public Integer getValidOrderNum(int member_id, String currency) {
		String sql = "";
		if(currency.equals("CNY")){
			 sql = "SELECT COUNT(*) from es_order o where o.member_id = ? AND o.parent_id is NOT NULL AND o.currency = '"+currency+"'  AND o.need_pay_money > 100 AND  o.status ="+OrderStatus.ORDER_COMPLETE;
		}else if(currency.equals("RUB")){
			 sql = "SELECT COUNT(*) from es_order o where o.member_id = ? AND o.parent_id is NOT NULL AND o.currency = '"+currency+"'  AND o.need_pay_money > 1000 AND  o.status ="+OrderStatus.ORDER_COMPLETE;
		}
	...
	@Override
	public RouteOrder getrouteorder(String orderid) {
		String  sql="select * from  es_route_order where  orderid=?";
		return (RouteOrder) this.baseDaoSupport.queryForObject(sql, RouteOrder.class, orderid);
	}
	 ...
	 */
	@Override
	public void subNeedPayMoney2(Integer order_id, Double money) {
		String sql = "UPDATE es_order SET discount =discount + ? WHERE order_id = ?";
		this.baseDaoSupport.execute(sql,money,order_id);
	}

	@Override
	public void subNeedPayMoney1(Integer order_id, Double needPayMoney,
			Double everyOrderNeed) {
		String sql = "UPDATE es_order SET  discount = discount + ?,need_pay_money =  ? WHERE order_id = ?";
		this.baseDaoSupport.execute(sql,everyOrderNeed,needPayMoney,order_id);
		
	}
	@Override
	public void updateShippingMoney(Integer order_id, Double shippingAmount) {
		// TODO Auto-generated method stub
		String sql="update es_order set shipping_amount=? where order_id=?";
		this.baseDaoSupport.execute(sql, shippingAmount,order_id);
		
	}
	@Override
	public List<Map> getOrderIsSkill(Integer orderid) {
		Member member = UserConext.getCurrentMember();
		/*System.out.println(DateUtil.getDateline());*/
		String sql="select o.order_id,o.create_time,e.id from es_check_memberlogin AS e LEFT JOIN es_order AS o ON o.order_id=e.order_id where o.disabled=0 AND ?-o.create_time<=? AND (o.`status`=? or o.`status`=?) AND o.create_time>? AND o.store_id IS NOT NULL and e.is_order=0 and e.order_id=? and e.member_id=?";
		List<Map> list= daoSupport.queryForList(sql,DateUtil.getDateline(),1200,OrderStatus.ORDER_NOT_PAY,OrderStatus.ORDER_NOT_CONFIRM,1398873600,orderid,member.getMember_id());
		return list;
	}
	@Override
	public List<Order> queryforOrder(String sn) {
		sn=sn.substring(0, sn.length()-1);	
		String sql = " select * from es_order o where o.parent_id is not null and o.sn like '"+sn+"%' and o.is_bonus=1"; 
		List<Order> orderlist = this.baseDaoSupport.queryForList(sql, Order.class);
		return orderlist;
	}
	@Override
	public void subTotalDiscount(Integer order_id, double discount) {
		String sql = "UPDATE es_order SET  discount = discount + ? WHERE order_id = ?";
		this.baseDaoSupport.execute(sql, discount,order_id);
		
	}
	@Override
	public void subNeedPayMoneyDiscount(int order_id, Double money) {
		String sql = "UPDATE es_order SET need_pay_money = need_pay_money - ?,discount = discount + ? WHERE order_id = ?";
		this.baseDaoSupport.execute(sql, money,money,order_id);
	}
	@Override
	public void addInformationOrder(Orderinformation orderinformation) {
		this.baseDaoSupport.insert("es_orderinformation", orderinformation);
		
	}
	@Override
	public void addGoodInformationo(Goodsinformation goodsinformation) {
		this.baseDaoSupport.insert("es_goodsinformation", goodsinformation);
	}
	@Override
	public void addOrderFeeInformation(OrderFeeInformation orderFeeInformation) {
		this.baseDaoSupport.insert("es_orderfeeinformation", orderFeeInformation);
	}
