...
public class PaymentManager extends BaseSupport<PayCfg> implements IPaymentManager {
	...
	public Integer add(String name, String type, String biref,String locale,
	...
	public void edit(Integer paymentId, String name,String type, String biref,String locale,
			Map<String, String> configParmas) {
	...
	public void subNeedPayMoney(int order_id, Double money) {
		String sql = "UPDATE es_payment_logs SET money = money - ?  WHERE order_id = ?";
		this.baseDaoSupport.execute(sql, money,order_id);
	}


	@Override
	public List listByLocale(String locale) {
		String sql = "select * from payment_cfg where locale = ? ";
		return this.baseDaoSupport.queryForList(sql, PayCfg.class,locale);
	}
	 ...
	 */
	@Override
	public void subNeedPayMoney1(Integer order_id, Double money) {
		String sql = "UPDATE es_payment_logs SET money =  ?  WHERE order_id = ?";
		this.baseDaoSupport.execute(sql, money,order_id);
	}
