...
//			this.json= paymentPlugin.onCallBack(ordertype);
			...
			if(result!=null && !result.equalsIgnoreCase("sentApp")){
				this.json= "<script>location.href='" + "/"+ordertype+"_"+pluginid+"_"+"payment-result.html" + "';</script>";
				this.logger.debug("支付回调结果"+json);
				return this.JSON_MESSAGE;
				
			}else{
				this.json="<script>parent.postMessage(\"成功\", \"*\");</script>";
				this.logger.debug("支付回调结果"+json);
				return this.JSON_MESSAGE;
			}
