...
public class CommentApiAction extends WWAction {
	...
	public String add(){
		...
		String showMessage=this.getText("comment.showMessage");
		String excuteMessage=this.getText("comment.excuteMessage");
		String publishMessage=this.getText("comment.publishMessage");
		/*if (language=="zh") {
			showMessage="对不起,只能上传gif,jpg,bmp,png格式的图片！";
			excuteMessage="对不起,图片不能大于200K！";
			publishMessage="发表成功";
		}else {
			showMessage="Извините вы только можете загрузить изображения в GIF, jpg, bmp, PNG формат！";
			excuteMessage="Извиниете картинка не может быть больше чем 200K！";
			publishMessage="Успешно опубликован";
		}*/
		if(file!=null){
			...
			if (!fileFileName.trim().equals("") && fileFileName.length() > 0) {
				...
				if(allowTYpe.toString().indexOf(ex.toLowerCase()) < 0){
					this.showErrorJson(showMessage);
			...
			if(file.length() > 200 * 1024){
				this.showErrorJson(excuteMessage);
				return this.JSON_MESSAGE;
			}
		...
		//判断是不是评论或咨询
		String xitong=this.getText("comment.xitong");
		if(type != 1 && type != 2){
				this.showErrorJson(xitong);
			return this.JSON_MESSAGE;
		}
		memberComment.setType(type);
		
		//int goods_id = StringUtil.toInt(request.getParameter("goods_id"),0);
		String noexist=this.getText("comment.noexist");
		if(goodsManager.get(goods_id) == null){
				this.showErrorJson(noexist);
			return this.JSON_MESSAGE;
		}
		memberComment.setGoods_id(goods_id);
		
		//String content = request.getParameter("content");
		String pinglun=this.getText("comment.pinglun");
		String shuru=this.getText("comment.shuru");
		if(StringUtil.isEmpty(content)){
				this.showErrorJson(pinglun);
			return this.JSON_MESSAGE;
		...
		String chengong=this.getText("comment.chengong");
		if(type == 1){
			if(member == null){
					this.showErrorJson(chengong);
				return this.JSON_MESSAGE;
			}
		...
		try {
			...
			this.showSuccessJson(publishMessage);
			
		} catch (RuntimeException e) {
			this.logger.error("发表评论出错",e);
			String fabiao=this.getText("comment.fabiao");
				this.showErrorJson(fabiao+e.getMessage());
		}
	...
	public String update(){
		...
		Locale locale = (Locale) session.getAttribute("locale");
		String language = locale.getLanguage();*/
		String gengxinSuccess=this.getText("comment.gengxinSuccess");
		String gengxinFail=this.getText("comment.gengxinFail");
		try {
			MemberComment memberComment = new MemberComment();
			...
			memberCommentManager.update(memberComment);
				this.showSuccessJson(gengxinSuccess);
		} catch (RuntimeException e) {
			this.logger.error("修改评论出错",e);
				this.showErrorJson(gengxinFail);
		}
		return this.JSON_MESSAGE;
	}
