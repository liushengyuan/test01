package com.enation.app.shop.component.payment.plugin.payeaseApp;
 ...
 */
@Component
public class PayeasePluginApp extends AbstractPaymentPlugin implements
		IPaymentEvent {
	...
	public PayeasePluginApp() {
		super();
	}

	public PayeasePluginApp(IPluginBundle pluginBundle) {
		pluginBundle.registerPlugin(this);
	}

	@Override
	public String getId() {
		return "payeasePluginApp";
	}
	...
	public String onPay(PayCfg payCfg, PayEnable order) {
		...
		//客户唯一标识
		String v_userref = request.getParameter("v_userref");
		...
		sParaTemp.put("v_userref", v_userref);
		//System.out.println("v_action_url="+v_action_url);
		String tempUrl = buildRequest(sParaTemp, v_action_url);
		return "{\"result\":1,\"data\":\""+tempUrl+"\"}";
	}
...
//		boolean verifiedMd5info2 = verifyMd5infoMd5(v_md5info, signature, md5Info);
	...
	public String onBackgroundCallBack(String ordertype) {
		...
		try {
			v_pmode = java.net.URLDecoder.decode(v_pmode, "GBK");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		String v_pstatus = request.getParameter("v_pstatus");
