...
public class MemberPointPlugin extends AutoRegisterPlugin implements
	...
	public void onRegister(Member member) {
		 ...
		 */
		if (true){
			//memberPointManger.checkIsOpen(IMemberPointManger.TYPE_REGISTER) 
//			RegisterSendPoint registerSendPoint = memberPointManger.getRegisterSendPoint();
//			long time = new Date().getTime()/1000;
//			int point = 0;
//			int mp = 0;
//			if(registerSendPoint != null ){
//				if(registerSendPoint.getStart_time()<=time && time<=registerSendPoint.getEnd_time()){
//					point = registerSendPoint.getPoint();
//					mp = registerSendPoint.getMp();
//				}
//			}
			int point = 0;
			int mp = 0;
			List<RegisterSendPoint> list = memberPointManger.getAllRegisterSendPoint();
			if(list !=null && list.size() > 0){
				for (RegisterSendPoint registerSendPoint : list) {
					point = registerSendPoint.getPoint();
					mp = registerSendPoint.getMp();
