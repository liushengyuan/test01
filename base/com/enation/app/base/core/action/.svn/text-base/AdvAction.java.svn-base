...
@Results({
	...
	@Result(name="adOfIndex", type="freemarker", location="/core/admin/adv/adOfIndex_list.html"),
	@Result(name="editAdOfIndex", type="freemarker", location="/core/admin/adv/adOfIndex_edit.html")
})
public class AdvAction extends WWAction {
	...
	private AdIndex adIndex;
	private Integer adIndexId;
	private Integer adIndexDisable;
	...
	public String adIndexJson() {
		this.webpage = adIndexManager.getAdIndexPage(this.getPage(), this.getPageSize());
		this.showGridJson(webpage);
		return this.JSON_MESSAGE;
	}
	
	public String editAdIndex() {
		adIndex = adIndexManager.getSingleAdIndex(adIndexId);
		if(adIndex.getAdindex_url()!=null&&!StringUtil.isEmpty(adIndex.getAdindex_url())){			
			imgPath = UploadUtil.replacePath(adIndex.getAdindex_url());
		}
		return "editAdOfIndex";
	}
	
	public String editAdIndexJson() {
		if (pic != null) {
			if (FileUtil.isAllowUp(picFileName)) {
				String path = UploadUtil.upload(this.pic,this.picFileName, "adv");
				adIndex.setAdindex_url(path);
			} else {
				this.showErrorJson("不允许上传的文件格式，请上传gif,jpg,bmp,swf格式文件。");
				return JSON_MESSAGE;
			}
		}
		adIndexManager.modifyAdIndex(adIndex);
		this.showSuccessJson("修改成功");
		return this.JSON_MESSAGE;
	}
	
	public String modifyAdIndexStatus() {
		adIndexManager.modifyAdIndexStatus(adIndexId,adIndexDisable);
	...
	public void setAdIndex(AdIndex adIndex) {
		this.adIndex = adIndex;
	}

	public Integer getAdIndexId() {
		return adIndexId;
	}

	public void setAdIndexId(Integer adIndexId) {
		this.adIndexId = adIndexId;
	}

	public Integer getAdIndexDisable() {
		return adIndexDisable;
	}

	public void setAdIndexDisable(Integer adIndexDisable) {
		this.adIndexDisable = adIndexDisable;
	}
