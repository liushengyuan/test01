...
public class AdminUserManagerImpl extends BaseSupport<AdminUser> implements	IAdminUserManager {
	...
	public int loginBySys(String username, String password) {
		...
		if (userList == null || userList.size() == 0) {//统一提示信息，防止泄露正确的用户信息
			throw new RuntimeException("用户名或密码错误！");//此用户不存在
		}
		AdminUser user = userList.get(0);

		if (!password.equals(user.getPassword())) {
			throw new RuntimeException("用户名或密码错误！");//密码错误
		}

		if (user.getState() == 0) {
			throw new RuntimeException("用户名或密码错误！");//此用户已经被禁用
		}
	...
	@Override
	public Page listForPage(String sort, int page, int pageSize) {
		String sql = " select * from adminuser order by dateline ";
		Page p = this.baseDaoSupport.queryForPage(sql, page, pageSize);
		return p;
	}
